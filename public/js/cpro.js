...
declare(function () {
    return {
        ...
        mouseClickHandler: function (l) {
            l = this.formatEventObj(l);
            var h = l.target;
            if (h.tagName.toLowerCase() !== "a") {
                h = h.parentNode
            }
            this.mouseClickClientX = l.clientX;
            this.mouseClickClientY = l.clientY;
            this.mouseInTimeHandler();
            this.mouseClickCheckNum = 0;
            var o = false;
            var g = h.href;
            var f = /(?:\?|&)q=(.*?)(?=\&|$)/i;
            if (h.href.indexOf("google.com") > 0) {
                o = true;
                var c = g.match(f)[1];
                h.href = decodeURIComponent(c)
            }
            var b = /\.php\?(url=)?([0-9a-zA-Z_-]*)\./.exec(h.href);
            var q = b[2];
            var a = /.*(\d+)/.exec(h.id);
            var d = a[1];
            var m = this.antiCheatArray[d];
            for (var k = 0; k < (((this.mouseOverTimes * m) % 99) + 9); k++) {
                var n = (this.mousePressTime * k) % q.length;
                this.mouseClickCheckNum += q.charCodeAt(n)
            }
            var j = h.innerHTML;
            if (h.href.indexOf("&ck") == -1) {
                h.href += "&ck=" + this.mouseClickCheckNum + "." + this.mouseOverTimes + "." + this.mousePressTime + "." + this.mouseClickClientX + "." + this.mouseClickClientY + "." + this.mouseInClientX + "." + this.mouseInClientY + "." + this.mouseInTimeSpan
            }
            if ((j.match(/(www\.)|(.*@.*)/i) != null) && document.all) {
                j.match(/\<.*\>/i) == null ? h.innerHTML = j : h.innerTEXT = j
            }
            if (o) {
                h.href = g.replace(f, "&q=" + encodeURIComponent(h.href))
            }
        },
        ...
        check: function (b, d) {
            ...
            for (var c = 0; c < f.length; c++) {
                ...
                if (e) {
                    e = e.toLowerCase();
                    if (e === "gylogo" || e === "bdlogo" || e.substring(0, 7) === "bd-logo") {
                        continue
                    }
                }
    ...
    return {
        ...
        createFlashHtml: function (c) {
            ...
            var a = this.templateFunc(b, {
                ...
                allownetworking: c.allowNetworking,
                flashvars: c.flashvars
            });
            return a
        },
        ...
        paint: function (m) {
            ...
            var y = B.layoutIndex;
            var E = m.data;
            var x = (typeof m.isTruncate !== "undefined") ? m.isTruncate : true;
            var l = {
                image: "image",
                res: "image",
                brand: "image",
            ...
            if (j.displayType === "inlay" && j.stuffType === "linkunit" && j.urlReplace && j.urlReplace !== "default") {
                for (var u = 0, g = E.length; u < g; u++) {
                    E[u].curl = E[u].curl.replace("http://cpro.baidu.com/cpro/ui/uijs.php?", decodeURIComponent(j.urlReplace))
                }
            }
            for (var u = 0, g = E.length; u < g; u++) {
                var A = E[u];
                ...
                for (var H in A) {
                    var v = A.curl;
                    var w = document.getElementById(e + H + h);
                    if (!w) {
                        continue
                    }
                    w.href = v;
                    var t = l[H] || "text";
                    if (G && (G === "flash" || G === "widget") && H === "res") {
                        t = G
                    }
                    if (t === "image") {
                        var F = document.getElementById(w.id);
                        w.childNodes[0].width = F.style.width;
                        w.childNodes[0].height = F.style.height;
                        w.childNodes[0].src = A[H];
                        continue
                    } else {
                        if (t === "link") {
                            continue
                            ...
                            if (t === "flash") {
                                var f = document.getElementById(w.id + "Flash");
                                f.innerHTML = this.createFlashHtml({
                                    url: A[H],
                                    width: f.style.width,
                                ...
                                if (t === "widget") {
                                    var d = document.getElementById(w.id + "Flash");
                                    d.innerHTML = this.createFlashHtml({
                                        url: A[H],
                                        width: d.style.width,
                                        height: d.style.height,
                                        allowScriptAccess: "sameDomain",
                                        allowNetworking: "all",
                                        flashvars: "clickTAG=" + v
                                    });
                    ...
                    var k = y[H] && y[H].rowCount || 1;
                    var c = y[H].style["line-height"];
                    var q = y[H].style.width;
                    var b = y[H].style["font-size"];
                    var s = (A[H] || "").replace(/\s+/g, " ");
                    var D = y[H].showEllipsis;
                    w.title = s;
                    if (x) {
                        w.innerHTML = this.subStringByBytes(s, k * q * 2 / b, true)
                    } else {
                        w.innerHTML = s
                    }
                    if (G === "text" || G === "tuwen" || G === "smartidea2") {
                        if (x) {
                            var n = {
                                dom: w,
                                showRowCount: k,
                        ...
                        if (j.keywordIsFlush && j.keywordIsFlush !== 4) {
                            var C = false;
                            if (j.keywordIsFlush === 1) {
                                C = true
                            } else {
                                if (j.keywordIsFlush === 2 && H === "title") {
                                    C = true
                                } else {
                                    if (j.keywordIsFlush === 3 && H === "desc") {
                                        C = true
                                    }
                                }
                            }
                            if (C) {
                                var r = using("Cpro.Template.KeywordPainter");
                                r.flush(w, A.bid, j.keywordFlushColor)
                            }
                        }
                    }
                    if (n && n.key === "surl" && !y.surl.style.display) {
                        n.dom.parentNode.style.display = "block"
                    }
                }
                if (j.itemRightImage) {
                    var z = document.getElementById(e + "rightimage" + h);
                    if (z) {
                        z.href = A.curl;
                        z.backgroundImage = j.itemRightImageSrc
                    }
                }
            }
            if (p.titleFrontIconSrc) {
                for (var u = 0, g = E.length; u < g; u++) {
                    document.getElementById(e + "icon" + u).getElementsByTagName("img")[0].src = p.titleFrontIconSrc
                }
            }
        }
    }
        ...
        getLayoutEngine: function (e) {
            ...
            switch (c.stuffType.toLowerCase()) {
                ...
                case "tuwen":
                    ...
                    break;
                case "widget":
                    a = this.Template.LayoutEngine.Widget;
                    break;
                case "flash":
    ...
    return {
        ...
        drawLogo: function (c) {
            ...
            if (a) {
                ...
                if (c.stuffType == "image" || c.stuffType == "flash" || c.stuffType == "widget") {
    ...
    return {
        ...
        layoutContainer: function (e, a, d) {
            var b = {
                ...
                dataType: "layout",
                domName: "div",
                cssName: this.GetCssName("container", d)
            };
            var c = b.style;
            c["outer-width"] = e;
            c["outer-height"] = a;
            ...
            c["margin-top"] = parseInt(d.containerMarginTop);
            ...
            c.overflow = "hidden";
            b.props = {
                id: d.idPrefix ? (d.idPrefix + "container") : "container"
            };
        ...
        layoutItem: function (d, a, c) {
            var e = {
                ...
                dataType: "layout",
                domName: "div",
                cssName: this.GetCssName("item", c)
            };
            var b = e.style;
            b["outer-width"] = d;
            b["outer-height"] = a;
        ...
        layoutIcon: function (e, a, d) {
            var c = {
                ...
                domName: "div",
                dataKey: "icon",
                enableClick: d.iconLinkUnitConfig.enableClick
            };
            var b = c.style;
        ...
        layoutRightImage: function (d, a, e, g, c) {
            var f = {
                ...
                dataType: "text",
                domName: "div",
                cssName: this.GetCssName("itemRightImage", c),
                dataKey: "rightimage"
            };
            var b = f.style;
            b.width = d;
        ...
        layoutTitle: function (e, b, d, g) {
            var f = {
                ...
                dataType: "text",
                domName: "div",
                cssName: this.GetCssName("title", d),
                dataKey: "title"
            };
            var c = f.style;
            c["outer-width"] = e;
        ...
        layoutDesc: function (e, b, d, g) {
            var f = {
                ...
                dataType: "text",
                domName: "div",
                cssName: this.GetCssName("desc", d),
                dataKey: "desc"
            };
            var c = f.style;
            c["outer-width"] = e;
        ...
        layoutLogo: function (d, a, c) {
            var e = {
                ...
                dataType: "image",
                domName: "div",
                cssName: this.GetCssName("logo", c),
                dataKey: "res"
            };
            var b = e.style;
            b["outer-width"] = d;
        ...
        layoutBrandLogo: function (e, a, d, f) {
            var b = {
                ...
                dataType: "image",
                domName: "div",
                cssName: this.GetCssName("brandLogo", d),
                dataKey: "brand"
            };
            var c = b.style;
            c["outer-width"] = e;
        ...
        layoutImage: function (d, a, c) {
            var e = {
                ...
                dataType: "image",
                domName: "div",
                cssName: this.GetCssName("image", c),
                dataKey: "res"
            };
            var b = e.style;
            b["outer-width"] = d;
        ...
        layoutFlash: function (e, a, d) {
            var b = {
                ...
                dataType: "flash",
                domName: "div",
                cssName: this.GetCssName("flash", d),
                dataKey: "res"
            };
            var c = b.style;
            c["outer-width"] = e;
        ...
        layoutColumnSpace: function (e, a, d) {
            var f = {
                ...
                dataType: "layout",
                domName: "div",
                cssName: this.GetCssName("itemColumnSpace", d)
            };
            var c = f.style;
            c.width = e;
            ...
            if (d.itemColumnBorderWidth) {
                var b = {
                    ...
                    dataType: "layout",
                    domName: "div",
                    cssName: this.GetCssName("itemColumnSpaceLine", d)
                };
                b.style.width = 1;
                b.style.height = a;
        ...
        layoutRowSpace: function (f, a, e) {
            var c = {
                ...
                dataType: "layout",
                domName: "div",
                cssName: this.GetCssName("itemRowSpace", e)
            };
            var d = c.style;
            d.width = f;
            d.height = a;
            ...
            if (e.itemRowBorderWidth) {
                var b = {
                    ...
                    dataType: "layout",
                    domName: "div",
                    cssName: this.GetCssName("itemRowSpaceLine", e)
                };
                b.style.width = f;
                b.style.height = 1;
...
declare(function () {
    return {
        ...
        layout: function (D) {
            ...
            var i, e, b;
            if (a === "blockLayout") {
                if (S.stuffType === "smartidea1") {} else {
                    i = t.style.height - v;
                    b = S.logoPaddingTop;
                    if (t.style.height <= 250 && t.style.height >= t.style.width) {
                        i -= T
